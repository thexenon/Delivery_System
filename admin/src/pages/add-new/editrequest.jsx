...
export default function EditRequest() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetchItem('serviceRequests', id).then((res) => {
        ...
        if (res?.data?.data) {
          setForm({
            status: res.data.data.data.status || 'pending',
            preference: res.data.data.data.preference || '',
            address: res.data.data.data.address || '',
            customer: res.data.data.data.customer?._id,
            service: res.data.data.data.service?._id,
            artisan: res.data.data.data.artisan?._id,
            artisanShop: res.data.data.data.artisanShop?._id,
            scheduledTime: new Date(res.data.data.data.scheduledTime)
              .toISOString()
              .slice(0, 19),
            location: res.data.data.data.location || {
              type: 'Point',
              coordinates: [0, 0],
            },
          });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    await submitUpdate(form, `serviceRequests/${id}`);
    setLoading(false);
    navigate('/request');
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4">Edit Service Request</h2>

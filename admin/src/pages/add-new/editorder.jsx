...
import { submitUpdate, fetchItems, deleteItem } from '../../services/user_api';
...
export default function EditOrder() {
  ...
  const [orderItemIdMap, setOrderItemIdMap] = useState({}); // { [productId]: orderItemId }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Submit order first
      await submitUpdate(data, `orders/${UID}`);
      // Fetch current order items from backend (to get their IDs)
      const orderItemsRes = await fetchItems(`orderitems?order=${UID}`);
      const existingOrderItems = orderItemsRes.data.data.data || [];
      const existingProductIdToOrderItem = {};
      existingOrderItems.forEach((item) => {
        const pid = item.product?._id || item.product;
        existingProductIdToOrderItem[pid] = item;
      });
      // Track which order item IDs are updated/created
      const updatedOrderItemIds = new Set();
      // For each product in the form, update or create order item
      for (const productId of products) {
        const product = allProducts.find((p) => p._id === productId);
        if (!product) continue;
        ...
        if (existingProductIdToOrderItem[productId]) {
          // PATCH existing order item
          const orderItemId = existingProductIdToOrderItem[productId]._id;
          await submitUpdate(orderItem, `orderitems/${orderItemId}`);
      ...
      for (const pid in existingProductIdToOrderItem) {
        if (!products.includes(pid)) {
          const orderItemId = existingProductIdToOrderItem[pid]._id;
          await deleteItem('orderitems', { _id: orderItemId });
        }
      }
      ...
      setError(
        err?.response?.data?.message ||
          'Failed to update order. Please check all fields.'
      );
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    async function fetchOrder() {
      try {
        const res = await fetchItems(`orders/${UID}`);
        const data = res.data.data.data;
        setCustomer(data.user?._id || data.user || '');
        setStatus(data.status || 'pending');
        setRider(data.rider?._id || data.rider || '');
        setTotal(data.totalAmount || data.total || 0);
        setPayment(data.payment || 'Momo');
        // Location
        ...
        // Products and product-related fields
        // If order has order items, fetch them
        const orderItemsRes = await fetchItems(`orderitems?order=${UID}`);
        const orderItems = orderItemsRes.data.data.data || [];
        const productIds = orderItems.map(
          (item) => item.product?._id || item.product
        );
        setProducts(productIds);
        // Map productId to orderItemId
        let idMap = {};
        orderItems.forEach((item) => {
          const pid = item.product?._id || item.product;
          idMap[pid] = item._id;
        });
        setOrderItemIdMap(idMap);
        // Set per-product fields
        let pq = {},
          pp = {},
          pv = {},
          po = {},
        ...
        orderItems.forEach((item) => {
          const pid = item.product?._id || item.product;
          pq[pid] = item.quantity || 1;
          pp[pid] = item.preference || '';
          pv[pid] = item.variety || '';
          // Options: handle both required and not required
          if (item.options && typeof item.options === 'object') {
            if (Array.isArray(item.options)) {
              // Not required: [{name, options: {optionname, quantity}}]
              item.options.forEach((opt) => {
                if (opt.name && opt.options) {
                  if (!po[pid]) po[pid] = {};
                  if (!oq[pid]) oq[pid] = {};
                  if (!oq[pid][opt.name]) oq[pid][opt.name] = {};
                  po[pid][opt.name] = po[pid][opt.name] || [];
                  po[pid][opt.name].push(opt.options.optionname);
                  oq[pid][opt.name][opt.options.optionname] =
                    opt.options.quantity;
                }
              });
            } else {
              // Required: {optionName: {value, quantity}}
              Object.entries(item.options).forEach(([optName, optObj]) => {
                if (optObj && typeof optObj === 'object') {
                  if (!po[pid]) po[pid] = {};
                  if (!oq[pid]) oq[pid] = {};
                  if (!oq[pid][optName]) oq[pid][optName] = {};
                  po[pid][optName] = optObj.value;
                  oq[pid][optName][optObj.value] = optObj.quantity;
                }
              });
            }
          }
        ...
        setProductPreferences(pp);
        setProductVarieties(pv);
        setProductOptions(po);
        setOptionQuantities(oq);
      } catch (err) {

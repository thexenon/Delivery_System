...
export default function AddShop() {
  ...
  useEffect(() => {
    fetchItems('users?role=artisan').then((res) => {
  ...
  const uploadImageToCloudinary = async (file) => {
    ...
    formData.append('public_id', `upload_${form.name}_${form.artisan}`);
    const res = await fetch(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await submitPost(shopdata, 'artisanshops').then((res) => {
        if (res.status == 201 && res.status == 200) {
          alert('Artisan Shop added successfully');
          navigate('/shop');
          setLoading(false);
        } else {
          setLoading(false);

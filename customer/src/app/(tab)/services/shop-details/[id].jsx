...
function StarRating({ rating, total }) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 6 }}>
      {stars}
      <Text
        style={{ color: COLORS.gray, marginLeft: 4, fontSize: SIZES.small }}
      >
        ({total})
      </Text>
    </View>
...
export default function ShopDetailsScreen() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const shopRes = await getItemById('artisanshops', id);
    setShop(shopRes.data.data.data);
    setServices(shopRes.data.data.data.services || []);
    setReviews(shopRes.data.data.data.reviews || []);
    setLoading(false);
  };
  ...
  const workingHoursArr = Array.isArray(shop.workingHours)
    ? shop.workingHours
    : [];
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.heroBanner}>
        <Image
          source={{ uri: shop.image || images.logo }}
      ...
      <View style={styles.tabContent}>
        {activeTab === 'About' && (
          ...
          >
            {/* Shop info card: address, phone, email, map, working hours, socials, contact/directions */}
            <View style={styles.infoCard}>
              ...
              <View style={{ flexDirection: 'row', marginTop: 8 }}>
                <TouchableOpacity
                  ...
                  onPress={() => Linking.openURL(`tel:+233${shop.phone}`)}
                >
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push(`/services/service-details/${item._id}`)
                }
                activeOpacity={0.85}
              >
                <Image
            ...
            ListEmptyComponent={
              <ErrorView msg="No services found for this shop." />
            }
        ...
        {activeTab === 'Reviews' && (
          <FlatList
            ...
            ListEmptyComponent={
              <ErrorView msg="No reviews yet for this shop." />
            }

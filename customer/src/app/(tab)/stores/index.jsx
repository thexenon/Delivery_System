...
function getTodayWorkingHours(workingHours) {
  ...
  return workingHours?.find((w) => w.day === today);
}
...
function isStoreOpen(workingHours) {
  ...
  if (now >= openTime && now < closeTime) {
    return { open: true, close: todayHours.close };
  } else if (now < openTime) {
    return { open: false, nextOpen: todayHours.open };
  } else {
...
export default function StoresScreen() {
  ...
  const fetchData = async () => {
    setLoading(true);
    setError(null);
    const res = await getItems('stores');
    if (res.status !== 200) {
      setError(res.message || 'Failed to fetch stores');
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (sort === 'top') {
      ...
      data.sort((a, b) => {
        const aOpen = isStoreOpen(a.workingHours).open ? 1 : 0;
        const bOpen = isStoreOpen(b.workingHours).open ? 1 : 0;
        return bOpen - aOpen;
      });
    } else if (sort === 'az') {
  ...
  const renderStore = ({ item }) => {
    const status = isStoreOpen(item.workingHours);
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => router.navigate(`/stores/store-details/${item._id}`)}
        activeOpacity={0.85}
      >
        <Image
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.address}>Phone: +233-{item.phone}</Text>
          <Text style={status.open ? styles.open : styles.closed}>
            {status.open
              ? `Open now (closes at ${status.close})`
              : status.nextOpen
              ? `Closed (opens at ${status.nextOpen})`
              : 'Closed'}
          </Text>
          <TouchableOpacity
            style={styles.mapBtn}
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.heroBanner}>
        <Text style={styles.heroTitle}>Browse Stores</Text>
        <Text style={styles.heroSubtitle}>Find the best stores near you</Text>
        <Text style={styles.heroSubtitle}>
          Call/Text us on +233593354286 if you want to list your store.
        </Text>
        ...
        <View style={styles.filterRow}>
          ...
          <TouchableOpacity
            style={[
              styles.filterBtn,
              sort === 'open' && styles.filterBtnActive,
            ]}
            onPress={() => setSort('open')}
          >
            <Text style={styles.filterBtnText}>Open Now</Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Image
                source={{ uri: item.image || images.logo }}
                style={styles.topStoreLogo}
              />
              <Text style={styles.topStoreName}>{item.name}</Text>
              <StarRating rating={item.ratingsAverage || 0} />
            </TouchableOpacity>
          )}
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.error}>No stores found.</Text>}
